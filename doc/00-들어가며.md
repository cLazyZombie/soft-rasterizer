...
- 좌표는 열벡터와 왼손 object/world/view 공간을 사용한다. `+X`는 오른쪽, `+Y`는 위, `+Z`는 전방이며 view 카메라도 `+Z`를 본다. NDC 깊이는 `0..1`, 화면은 y-down이다. 세부 수식은 5-7장과 `docs/decisions/coordinates.md`에서 고정한다.
