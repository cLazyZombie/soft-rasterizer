...
_웹(JavaScript) + WebAssembly(Rust) 26장 교육과정 설계서_
...
- **5부 · 에셋, 품질, 검증, 성능**
  ...
  - [26장. GLB 장면, Skinning과 Animation](26-glb-장면-skinning-animation.md)
- **부록**
...
| 2부 | 6-10장 | 3D 변환과 가시성 | 카메라로 움직이는 여섯 평면 클리핑을 통과한 와이어프레임 |
| 3부 | 11-15장 | 삼각형 래스터 핵심 | 균열 없는 컬러 3D 큐브와 깊이 버퍼 |
| 4부 | 16-20장 | 텍스처/조명/입력 | 텍스처와 조명이 적용된 인터랙티브 모델 |
| 5부 | 21-26장 | 에셋/품질/성능 | 외부 모델과 animation, 품질 옵션, 테스트와 성능 보고서 |
...
- 초기 모델은 코드로 만든 큐브다. 21장에서 OBJ 기준선을 만들고 26장에서 GLB 2.0 scene, material, node animation과 skinning을 실전 확장선으로 연결한다.
...
each animation frame(timestamp):
  ...
  if buffer identity, pointer, byte length, width or height changed:
      rebuild Uint8ClampedArray view and ImageData dimensions
...
각 장은 앞 장의 결과를 입력으로 사용한다. 5장에서 행렬 곱을 익히고, 6–7장에서 점이 화면으로 가는 나눗셈을 계산한 뒤, 10장에서 나누기 전 clipping을 배운다. 11–13장에서 픽셀의 소유·가중치·깊이를 정하고, 14장에서 화면의 가중치를 표면의 가중치로 복원한다. 공식 옆의 숫자 예제를 먼저 손으로 따라간 뒤 구현·검증 항목을 읽으면 좋다.

코드 블록의 `Σ`는 지정한 항들을 모두 더하라는 뜻이다. `normalize(v)`는 길이로 나누기, `lerp(a,b,t)`는 `(1-t)a+tb`다. 벡터 성분에 적용하는 계산과 행렬 곱은 구분한다. 화면 y는 아래로 증가하지만 world/view y는 위로 증가한다.

위의 권장 디렉터리 구조는 책임을 설명하는 개념도다. 실제 checkout은 `renderer-core/src/lib.rs`, `math.rs`, `camera.rs`, `raster.rs` 등의 구현을 사용한다. 각 장의 ‘구현’ 설명은 해당 milestone을 뜻하고, ‘최신 경로’라고 적은 내용만 후속 장까지 포함한다. 실행본은 고정 revision, 이 교재는 현재 수정본이다.

첫 rAF의 이전 timestamp는 없으므로 dt=0으로 시작한다. 이후 `dt=clamp((timestamp-previous)/1000,0,0.1)`을 사용한다. 버퍼 원소 수가 같아도 2×6→3×4처럼 가로·세로가 바뀌면 ImageData 치수를 다시 구성해야 한다.
